...
x=1;                   %variable for recording movie frames
v=1;                   %variable for recording movie frames
w=1;                   %variable to track end effector position
s=[0];                 %To store end effector X coordinate position
p=[0];                 %To store end effector Y coordinate position
%----------------------------------------------------------------------

%Using Trigonometry for finding position of end-effector
for i=1:length(a)
    t1=a(i);
for j=1:length(b)
    t2=b(j);
...
x0=0;                         %x coordinate of base frame
y0=0;                         %y coordinate of base frame

x1=l1*cosd(t1);               %x coordinate of 1st joint
y1=l1*sind(t1);               %y coordinate of 1st joint

x2=x1+l2*cosd(t1+t2);         %x coordinate of end effector    
y2=y1+l2*sind(t1+t2);         %y coordinate of end effector

s(w)=x2;
p(w)=y2;
%--------------------------Plotting---------------------------------
f=plot([x0,x1],[y0,y1],[x1,x2],[y1,y2],s,p,'--c','linewidth',3);
f(3).LineWidth=1;
title('Forward Kinematics using Trignometry');
xlabel('X axis');
ylabel('Y axis');
...
M(x)=getframe(gcf);% to generate movie frame gcf is to get the current fig handle
%------------------------------------------------------------------------------------
...
movie(M)
vf=VideoWriter('forward_kinematics_trignometry','uncompressed avi');
open(vf)
writeVideo(vf,M)
close(vf)

movie(N)
vf=VideoWriter('forward_kinematics_DH_transform','uncompressed avi');
open(vf)
writeVideo(vf,N)
close(vf)